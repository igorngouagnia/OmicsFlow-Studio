...
ROOT_DIR = os.getcwd()
INPUT_DIR = os.environ.get("OMICS_IN_DIR", os.path.join(ROOT_DIR, "Protéomique"))
METADATA_PATH = os.environ.get("OMICS_META_PATH", os.path.join(INPUT_DIR, "metadata.tsv"))
PROTEINS_PATH = os.path.join(INPUT_DIR, "proteinGroups.tsv")
OUTPUT_DIR = os.environ.get("OMICS_OUT_DIR", os.path.join(ROOT_DIR, "Résultats", "Protéomique"))
...
def impute_missing_data(data):
    ...
    """
    # Paramètres standards Perseus : width=0.3, down_shift=1.8
    mu = data.mean()
    sigma = data.std()
    ...
    for col in data.columns:
        ...
        if n_nan > 0:
            # Décalage de la moyenne vers le bas (simule le seuil de détection)
...
def calculate_stats_refined(df, group_test_cols, group_ref_cols):
    ...
    # 2. Imputation (indispensable pour retrouver les protéines de la Table S8)
    ...
    # 4. T-test indépendant avec Pingouin (plus robuste pour faibles réplicats, Welch-s correction)
    import pingouin as pg
    def get_pval(row):
        val_test = data_test_imp.loc[row.name].dropna().astype(float)
        val_ref = data_ref_imp.loc[row.name].dropna().astype(float)
        if len(val_test) > 1 and len(val_ref) > 1:
            res = pg.ttest(val_test, val_ref, correction=True)
            return res['p-val'].values[0]
        return 1.0
        
    pvals = df.apply(get_pval, axis=1)
    
    # 5. Correction Benjamini-Hochberg (FDR)
    # On remplace les pvals NaN par 1.0 avant correction
    mask = ~pvals.isna()
...
def create_pptx_report(all_stats):
    ...
    slide.shapes.title.text = "Rapport d'Analyse Protéomique"
    slide.placeholders[1].text = f"Analyse Différentielle MTM1-a (Pipeline Papier)\nDate : {datetime.now().strftime('%d/%m/%Y')}"

    for age, stat in all_stats.items():
        slide = prs.slides.add_slide(prs.slide_layouts[5])
        slide.shapes.title.text = f"Synthèse Age : {age}"
        table_data = [
            ["Indicateur", "Valeur"],
    ...
    prs.save(os.path.join(OUTPUT_DIR, "Rapport_Proteomique_MTM1a.pptx"))
...
def main():
    ...
    for age in df_meta['Age'].unique():
        print(f"--- Analyse Age : {age} (Imputation + FDR) ---")
        
        cols_wt = df_meta[(df_meta['Age'] == age) & (df_meta['Genotype'] == 'WT')]['Sample'].tolist()
        cols_ko = df_meta[(df_meta['Age'] == age) & (df_meta['Genotype'] == 'KO')]['Sample'].tolist()
        cols_res = df_meta[(df_meta['Age'] == age) & (df_meta['Genotype'] == 'KO_Dnm2')]['Sample'].tolist()

        # Moyennes LFQ pour affichage
        final_df[f'Moyenne_LFQ_{age}_WT'] = df_prot[cols_wt].mean(axis=1)
        final_df[f'Moyenne_LFQ_{age}_KO'] = df_prot[cols_ko].mean(axis=1)
        final_df[f'Moyenne_LFQ_{age}_KO_Dnm2'] = df_prot[cols_res].mean(axis=1)
        ...
        # Seuils du papier : |Log2FC| > 1 et Adjusted P-value < 0.05
        ...
        patho_df.to_csv(os.path.join(OUTPUT_DIR, f"Proteomics_Signature_Patho_{age}.csv"), sep=";")
        
        rescue_df = final_df[final_df[f'Significatif_Rescue_{age}'] == 'OUI']
        rescue_df.to_csv(os.path.join(OUTPUT_DIR, f"Proteomics_Efficacite_Rescue_{age}.csv"), sep=";")
    ...
    final_df.to_csv(os.path.join(OUTPUT_DIR, "Proteomics_Analysis_Full_MTM1a.csv"), sep=";")
    
    if PPTX_AVAILABLE:
        create_pptx_report(report_stats)
        print(f"Rapport PowerPoint généré.")

    print(f"\nTerminé ! Résultats conformes au pipeline du papier dans : {OUTPUT_DIR}")
