...
def main():
    root_dir = os.getcwd()
    base_res = os.environ.get("OMICS_OUT_DIR", os.path.join(root_dir, "Résultats"))    
    ...
    # ==========================
    # 2. PROTÉOMIQUE (PYTHON)
    # ==========================
    pyp_dir = os.path.join(out_dir, "Proteomique_Python")
    os.makedirs(pyp_dir, exist_ok=True)
    in_pyp = os.path.join(base_res, "Protéomique", "Proteomics_Analysis_Full_MTM1a.csv")
    print("\n[Protéomique Python]")
    if os.path.exists(in_pyp):
        df = pd.read_csv(in_pyp, sep=";")
        name_col = 'Gene names'
        for age in ['E18.5', '2w', '7w']:
            create_volcano(df, name_col, f'Log2FC_Patho_{age}', f'Pvalue_Patho_{age}', 
                           f"Protéomique Python Patho ({age})", 
                           os.path.join(pyp_dir, f"Volcano_Patho_{age}.html"), lfc_th=1.0)
            
            create_volcano(df, name_col, f'Log2FC_Rescue_{age}', f'Pvalue_Rescue_{age}', 
    ...
    in_pr = os.path.join(base_res, "Protéomique", "R_DEP", "Proteomics_Analysis_Full_MTM1.csv")
    print("\n[Protéomique R (DEP)]")
    if os.path.exists(in_pr):
        df = pd.read_csv(in_pr, sep=";")
        name_col = 'Gene.names' if 'Gene.names' in df.columns else 'Gene names'
        for age in ['E18.5', '2w', '7w']:
            # P-value vs Padj adaptation
            p_col_patho = f'Padj_Patho_{age}' if f'Padj_Patho_{age}' in df.columns else f'Pvalue_Patho_{age}'
            p_col_rescue = f'Padj_Rescue_{age}' if f'Padj_Rescue_{age}' in df.columns else f'Pvalue_Rescue_{age}'
            
            create_volcano(df, name_col, f'Log2FC_Patho_{age}', p_col_patho, 
                           f"Protéomique R Patho ({age}) strict", 
                           os.path.join(pr_dir, f"Volcano_Patho_{age}.html"), 
                           pval_th=0.05, lfc_th=1.0)
            
            create_volcano(df, name_col, f'Log2FC_Rescue_{age}', p_col_rescue, 
                           f"Protéomique R Rescue ({age}) strict", 
