...
if menu == "📊 Dashboard":
    ...
    with col_search:
        st.text_input("🔍 Search Genes/Metabolites...", placeholder="Ex: MTM1, BIN1...")
    with col_view:
    ...
    with c_left:
        if files:
            f_sel = st.selectbox("Select Cohort File", files, key="dashboard_file")
            ...
            st.warning("No data found. Run a pipeline first.")
    ...
    with c_right:
        ...
        st.markdown("<div class='target-val'>1273</div>", unsafe_allow_html=True)
        st.markdown("<div class='target-label'>Cibles Rescue Validées</div>", unsafe_allow_html=True)
        st.markdown("<div class='target-val'>189</div>", unsafe_allow_html=True)
...
# ==========================================
    ...
    mf = st.file_uploader("Upload Data")
    if mf: 
...
# ==========================================
    ...
    # RNA Section
    style_rna = "active-section" if "Transcriptomics" in val_case else "grayed-out"
    ...
    # Proteo Section
    style_p = "active-section" if "Proteomics" in val_case else "grayed-out"
    ...
    cp = st.columns(2)
    with cp[0]:
        st.markdown(f"<div class='{'active-section' if '2w' in val_case else 'grayed-out'}'>", unsafe_allow_html=True)
        ...
        st.markdown("</div>", unsafe_allow_html=True)
    with cp[1]:
        st.markdown(f"<div class='{'active-section' if '7w' in val_case else 'grayed-out'}'>", unsafe_allow_html=True)
